...
c = conn.cursor()
conn2 = sqlite3.connect('db/msbot_tables_new.db')
d = conn2.cursor()
...
def send_new():
	t_conn = sqlite3.connect('db/msbot_tables_new.db')
	tc = t_conn.cursor()
	print('thread')
	spoilers = msbot.mslib.getLatestSpoilers()
	print('spoiler get')
	...
	#send spoilers to all users
	for user in current_users:
		for spoiler in spoilers:
			tc.execute('''
				SELECT id FROM spoilers WHERE id = ?
				''', (spoiler["id"],))
			print(spoiler["name"])
			if len(tc.fetchall()) == 0:
				print('senderoni')
				if user[1] == 1: #if user wants to receive images
					spoiler_json = {
						"message": {
							"attachment": {
								"type": "image",
	...
	for spoiler in spoilers:
		...
		print(spoiler["id"])
		if len(tc.fetchall()) == 0:
			if "card_faces" not in spoiler: #if card only has 1 face
				if "power" in spoiler and "toughness" in spoiler:
					power = spoiler["power"]
					toughness = spoiler["toughness"]
				else:
					power = None
					toughness = None
				print("pt")
				...
				tc.execute('''
					INSERT INTO spoilers VALUES(?,?,?,?,?,?,?,?,?)
					''', (spoiler["id"], spoiler["name"], spoiler["image_uris"]["png"], spoiler["mana_cost"], spoiler["type_line"], spoiler["oracle_text"], power, toughness, spoiler["scryfall_uri"]))
				t_conn.commit()
				print("db")
			if spoiler["layout"] == "split":
				faces = spoiler["card_faces"]
				for face in faces:
					if "power" in spoiler and "toughness" in spoiler:
						power = spoiler["power"]
						toughness = spoiler["toughness"]
					...
					tc.execute('''
						INSERT INTO spoilers VALUES(?,?,?,?,?,?,?,?,?)
						''', (spoiler["id"], face["name"], spoiler["image_uris"]["png"], face["mana_cost"], face["type_line"], face["oracle_text"], power, toughness, spoiler["scryfall_uri"]))
					t_conn.commit()
...
def handle_message(sender_psid, received_message):
	...
	if received_message:
		...
		if received_message.lower() == 'poop':
			...
			send_message(sender_psid, response)
		elif received_message.lower() == 'urmom':
			d.execute('''
				SELECT id FROM users WHERE id = ?
				''', (sender_psid,))
			if len(d.fetchall()) == 0:
				d.execute('''
					...
					''', (sender_psid, 1, 1, 1, 1, 1))
				conn2.commit()
				response = {TEXT: 'no ur mom'}
				send_message(sender_psid, response)
			else:
				response = {TEXT: 'stop'}
				send_message(sender_psid, response)
				spoilers = msbot.mslib.getLatestSpoilers()
		elif received_message.lower() == 'sendd':
			send_message(sender_psid, {TEXT:'sadf'})
			send_new()
		else:
			c.execute('''
				SELECT id FROM users WHERE id = ?
...
def webhook_event():
	...
	if req[OBJECT] == PAGE_OBJECT:
		for entry in req[ENTRY]:
			...
			if event[MESSAGE]:
				try:
					event_send = event[MESSAGE][TEXT]
				except KeyError:
					print('Non-text message received')
				handle_message(sender_psid, event_send)
			elif event[POSTBACK]:
